...
SKIP_EXPENSIVE_COMPUTING = FALSE
...
W = - 1/2 * A %*% D %*% t(A)
e = eigen(W)
V = e$vectors[,1:2] %*% diag(sqrt(e$values[1:2]))

#Offset the results so that category 1 is at the origin
V1 = t(t(V) - V[1,])
#rotate and scale the results so that category 2 is at (1,0);
theta = -asin(V1[2,1]/sqrt(V1[2,1]^2 + V1[2,2]^2)) + pi/2
r  = matrix(c(cos(theta),sin(theta),-sin(theta),cos(theta)), 2, 2)  #rotation matrix
V2 = V1 %*% r
V3 = V2 / V2[2,1]

plot(V3, xlab = "", ylab = "", col="Red", pch = 8, cex = 2, xlim = c(-0.1, 1.1))
for(i in 1:label_count){
  text(V3[i,1], V3[i,2], label_names[i,1], offset = 0)
}
```
