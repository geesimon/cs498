...
for (k in c(1, 5, 10, 15, 20, 100, 150, 200)){
  drawImage(cld(all_pca[[sample_label]]$pc, all_pca[[sample_label]]$mean,
                t(df_images[sample_index, -3073]), k), k)
}
```
...
for (i in 1:label_count){
  ...
  for (k in 1:pc_count){
    e[k] = pca_image_error(images, all_pca[[i]]$pc, all_pca[[i]]$mean, k)
...
plot(V3, xlab = "", ylab = "", col="Red", pch = 8, cex = 1.5, xlim = c(-0.1, 1.1))
for(i in 1:label_count){
...
for (i in 1:label_count) { images_by_category[[i]] = df_images[df_images$V3073 == i,] }

pb = txtProgressBar(min = 0, max = sum(1:10), style = 3)
pbc = 1
for (i in 1: label_count) {
  for (j in i : label_count){
    error_1 = pca_image_error(images_by_category[[i]], all_pca[[j]]$pc, 
                              all_pca[[i]]$mean, pc_count)
    if(i != j) {
      error_2 = pca_image_error(images_by_category[[j]], all_pca[[i]]$pc, 
