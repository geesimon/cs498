...
DocLength = rowSums(DocVectors)

Log_P = log(P)
Log_pi = log(pi)

#E Step
p_j = rep(0, TopicCount)
for (i in 1:DocCount) {
  for (j in 1:TopicCount) {
     p_j[j] = sum(Log_P[j,] * DocVectors[i,]) + Log_pi[j] //todo:optimize
  }
  
  Log_W[i,] = p_j - logSumExp(p_j)
}

W = exp(Log_W)
#M Step
P = (t(W) %*% DocVectors) / c(t(W) %*% DocLength)
pi = colSums(W) / DocCount
```
