...
GMM_EM = function(X, cluster.count, converge.threshold = 1e-7) {
  ...
  log_W = matrix(-10000, N, cluster.count)
  log_W_old = log_W
  old_W = matrix(0, N, cluster.count)
  #log_X = log(X)
  #log_pi = log(pi)
  #log_X[log_X < -10000] = -10000
  #n_zeros = matrix(0, N, 1)
  ...
  for (iter in 1:5) {
    ...
    for (j in 1:cluster.count) {
      D = t(X) - U[j]
      log_W[,j] = (-0.5 * colSums(D ^ 2)) + log_pi[j]
      
      #W[,j] = exp(-0.5 * colSums(D ^ 2)) * log_pi[j]
    }
    for (i in 1:N) {
      log_W[i, ] = log_W[i,] - logSumExp(log_W[i,])
...
X = img * 255
cluster.count = 10
converge.threshold = 1e-7

r = GMM_EM(X, 10)

segment_image("smallstrelitzia.jpg", 10)
```
