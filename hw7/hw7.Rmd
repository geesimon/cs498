...
$$
\omega_{i,j}=\frac{\left[\prod_{k} p_{j,k}^{x_{i,k}}\right]\pi_j}{ \sum_{l} \left[\prod_{k} p_{j,k}^{x_{i,k}}\right]\pi_l}
$$
We transform this equation to logsumexp form to benefit calculation
$$
\log(\omega_{i,j})=\sum_{k}(\log p_{j,k}^{x_{i,k}}) + \log\pi_j - LogSumExp_l(\sum_{l,k} (\log p_{j,k}^{x_{i,k}}) + \log\pi_{l})
$$
