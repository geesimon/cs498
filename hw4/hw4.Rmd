...
- From some height, this clustering makes Turkey, Greece and Yugoslavia one cluster. Romania, Czechoslovakia, USSR etc a second cluster and the rest counties a 3rd cluster If we check the history, countries in the second cluster are most socialism countries at that time (except Spain, Portugal and Ireland) and also close to each other. Countries in the 3rd cluster are  capitalism countries except East Germany which is closely grouped with West Germany (since they were one country). 
...
# AllTrainDF = data.frame()
# for (i in 1:length(TrainData)){
#   for (j in 1:length(TrainData[[i]])){
#     AllTrainDF = rbind(AllTrainDF, TrainData[[i]][[j]])
#   }
# }
...
We vector quantize training and test dataset and compute the histgram of these dataset to build features for later classification.
```{r}
...
```{r, eval=!params$load_precomputed_data}
cluster_number = c(300, 480, 500, 700, 800, 900, 1000)
accuracies = rep(0, length(cluster_number))

for (i in 1:length(cluster_number)){
  km = kmeans(AllTrainDF, centers = cluster_number[i])
...
We see cluster of $`r improve_result_1[["cluster_number"]][which.max(improve_result_1[["accuracies"]])]`$ gives us the best accuracy $`r max(improve_result_1[["accuracies"]])`$.
...
```
We see sample size of $`r improve_result_2[["sample_size"]][which.max(improve_result_2[["accuracies"]])]`$ gives us the best accuracy $`r max(improve_result_2[["accuracies"]])`$.
