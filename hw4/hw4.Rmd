...
calculate_accuracy = function(train_frame, test_frame, ntrees, max_depths) {
  ...
  return (list(accuracies = accuracies, models = models))
}
...
result = calculate_accuracy(h2o_train_data, h2o_test_data, c(10, 20, 30), c(4, 8, 16))

```
- Error rate: $`r max(result$accuracies)`$

Confusion Matrix
```{r}
h2o.confusionMatrix(result$models[[which.max(result$accuracies)]])
```
