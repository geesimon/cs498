...
**Abswer:** The plots of (c) and (d) do look follow linearity assumption better than (a) and (b). In addition, (c) and (d) don't seem have much difference, since (c) has fewer variables and simpler, we would choose **(c)** as the best regression model.  
...
- The experiment shows that model (a) and (d) can be slightly improved by using a regularizer with $\lambda$ of `r model_a_1$lambda.min` and `r model_b_1$lambda.min` respectively.
