...
plot(log(brunhild_df$Hours), log(brunhild_df$Sulfate), xlab = "LOG(Hours)", ylab = "LOG()", col = "dodgerblue", pch = 20, cex = 1.5)
...
curve(exp(model_a$coefficients[1] + log(x) * model_a$coefficients[2]), from = min(brunhild_df$Hours), to = max(brunhild_df$Hours), add = TRUE, col = "darkorange", lwd = 2)
```
...
plot(model_b$fitted.values, model_b$residuals, col = "dodgerblue", 
     pch = 20, cex = 1.5, xlab = expression(paste("Fitted Value of ",Mass^(1/3))), ylab = "Residuals")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
```
```{r}
...
qqnorm(physical_df$Mass - model_b$fitted.values ^ 3, main = "Normal Q-Q Plot, model (b)", col = "dodgerblue")
...
plot(model_a$fitted.values, model_a$residuals, col = "dodgerblue", 
     pch = 20, cex = 1.5, xlab = "Fitted values of Age", ylab = "Residuals (without gender)")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
```
...
```{r}
abalone_data_1 = abalone_data
#abalone_data_1$V1 = as.integer(abalone_data_1$V1)
#abalone_data_1$V1[abalone_data_1$V1 == 3] = -1
#abalone_data_1$V1[abalone_data_1$V1 == 2] = 0

model_b = lm(V9 ~ ., data = abalone_data_1)
plot(model_b$fitted.values, model_b$residuals, col = "dodgerblue", 
...
plot(exp(model_c$fitted.values), abalone_data$V9 - exp(model_c$fitted.values), col = "dodgerblue", 
     pch = 20, cex = 1.5, xlab = "Fitted values of Age", ylab = "Residuals (without gender)")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
```
...
```{r}
model_d = lm(log(V9) ~ ., data = abalone_data_1)
plot(exp(model_d$fitted.values), abalone_data_1$V9 - exp(model_d$fitted.values), col = "dodgerblue", 
     pch = 20, cex = 1.5, xlab = "Fitted values of Age", ylab = "Residuals (without gender)")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
```

##(e) 
It turns out that determining the age of anabalone is possible, but difficult (you section the shell, and count rings). Use your plots to explain which regression you would use to replace this procedure, and why.
