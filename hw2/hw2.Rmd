...
#Normalize the data
for (i in 1:dim(adult_feature)[2]) {
  s = sd(adult_feature[,i])
  m = mean(adult_feature[,i])
  
  adult_feature[,i] = (adult_feature[,i] - m) / s
}
...
non_test_indexes = (1:total_count)[-test_indexes]
```
...
alpha = runif(dime(adult_feature)[2])
beta = runif(1)

for (epoch in 1:max_epoch) {
  train_indexes = sample(non_test_indexes, train_count)
  train_feature = adult_feature[train_indexes,]
  train_label   = adult_label[train_indexes]
  test_data     = adult_feature[-train_indexes,]
  test_label    = adult_label[-train_indexes]
    
  for (step in 1:max_step){
    sample_indexes = sample(train_indexes, sample_count)
  }
}
```
