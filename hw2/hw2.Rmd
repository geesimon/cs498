...
We will write a function to do Stochastic Gradient Descent, by using the following fomular. Its extension to text book's example and add support to multiple sample $N_b$.
$$
...
Note: we take 50 samples ($N_b = 30$) in order to make a good data coverage. Experiements also show lower the sample won't make much difference.
...
Let's make a boxplot of valication accuracy for each $\lambda$
```{r}
df = data.frame()
for (i in 1:length(lambdas)){
