...
def inference(images):
  ...
  # dropout 1
  dropped1 = tf.nn.dropout(pool1, 0.25)
  ...
  with tf.variable_scope('conv3', reuse=tf.AUTO_REUSE) as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         stddev=5e-2,
                                         wd=None)
    conv = tf.nn.conv2d(dropped1, kernel, [1, 1, 1, 1], padding='SAME')
  ...
  dropped2 = tf.nn.dropout(pool2, 0.25)

  # local3
  with tf.variable_scope('local3', reuse=tf.AUTO_REUSE) as scope:
    # Move everything into depth so we can perform a single matrix multiply.
    reshape = tf.reshape(dropped2, [images.get_shape().as_list()[0], -1])
