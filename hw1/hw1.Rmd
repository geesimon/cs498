...
load_label_data = function(file_name){
  ...
  training_data = load_image_data("./MNIST/train-images-idx3-ubyte")
  test_data = load_image_data("./MNIST/t10k-images-idx3-ubyte")
  sb_training_data = load_image_data("./MNIST/train-images-idx3-ubyte-sb")
  sb_test_data = load_image_data("./MNIST/t10k-images-idx3-ubyte-sb")
  training_label = load_label_data("./MNIST/train-labels-idx1-ubyte")
...
for(i in index)
{
  image(matrix(training_data[i,], 28, 28)[, 28:1], col  = gray((255:0)/255))
}

for(i in index)
{
  image(matrix(sb_training_data[i,], 28, 28)[, 28:1], col  = gray((255:0)/255))
}
```


##Part A
Investigate classifying MNIST using naive Bayes. Fill in the accuracy values for the four combinations of Gaussian v. Bernoulli distributions and untouched images v. stretched bounding boxes. Please use 20 x 20 for your bounding box dimensions.

```{r}
library(caret)

model = train(training_data, training_label, 'nb', trControl = trainControl(method='cv', number=5))
