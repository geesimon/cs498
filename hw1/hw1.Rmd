...
cross_validation = function(x, y, fold_num, method = "Gaussian") {
  ...
  for (i in 1:fold_num){
    ...
    models[[i]] = func_map[[method]]$fit(train_x, train_y)
    #pred_y = gnb.predict(models[[i]], valid_x)
    pred_y = func_map[[method]]$predict(models[[i]], valid_x)
...
best_model = cross_validation(train_data, train_label, 5, method = "Bernoulli")
pred_y = gnb.predict(best_model, test_data)
bernoulli_accuracy = sum(pred_y == test_label)/length(test_label)

best_model = cross_validation(sb_train_data, train_label, 5, method = "Bernoulli")
pred_y = gnb.predict(best_model, sb_test_data)
