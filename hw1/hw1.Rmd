...
ss_mnist_file = gzfile("./MNIST/t10k-images-idx3-ubyte-sb.gz", "wb")

writeBin(readBin(mnist_file, integer(), n = 4,  endian = "big"), ss_mnist_file)

for(i in 1:10){
  m = matrix(readBin(mnist_file, integer(), size = 1, n = 28*28, endian = "big", signed = FALSE), 28, 28);
  
  box = ss_box(m, 20, 20)
  writeBin(c(box), ss_mnist_file, endian = "big")
...
```{r}
ss_mnist_file = gzfile("./MNIST/t10k-images-idx3-ubyte-sb.gz", "rb")
readBin(ss_mnist_file, integer(), n = 4,  endian = "big")

for(i in 1:10){
  m = matrix(readBin(ss_mnist_file, integer(), size = 1, n = 28*28, endian = "big", signed = FALSE), 28, 28);
  
  #box = ss_box(m, 20, 20)
  #writeBin(c(box), ss_mnist_file)
  image(m[,dim(m)[2]:1], col  = gray((255:0)/255))
  #image(box[,dim(box)[2]:1], col  = gray((255:0)/255))
}
